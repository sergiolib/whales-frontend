    ...
    <v-container>
        <v-layout>
            <v-flex xs12>
                <v-card>
                    <v-card-title>
                        <h1>Pipelines manager</h1>
                    </v-card-title>
                    <v-card-text>
                        <p>This module allows the user to lunch or cancel the pipelines processes. It also shows the
                            results.</p>
                    </v-card-text>
                    <v-card-text>
                        <v-select
            ...
            <v-flex xs12 sm10 offset-sm1 v-if="pipeline_selected !== ''">
                <v-card>
                    <v-card-text>
                        ...
                        <v-flex>
                            <h5 class="text-xs-left">Owned by {{ loaded_pipeline.owner }}</h5>
                        </v-flex>
                    </v-card-text>
                    <v-card-text>
                        ...
                        <v-flex><p></p></v-flex>
                        <v-flex class="center">
                            <v-btn v-if="loaded_pipeline.public" @click="switch_public_state">Make private</v-btn>
                            <v-btn v-else @click="switch_public_state">Make public</v-btn>
                        </v-flex>
                        <v-card style="padding: 5px">
                            <v-list>
                                <v-subheader>
            ...
            <v-dialog width="500" v-model="rename_dialog">
                <v-card>
                    ...
                    <v-card-actions>
                        ...
                        <v-btn color="green darken-1" flat :disabled="rename_new_name.length === 0"
                               @click.native="rename_pipeline">Ok
                        </v-btn>
                    </v-card-actions>
                </v-card>
            </v-dialog>
    ...
    export default {
        ...
        methods: {
            ...
            switch_public_state() {
                ...
                let url = api_url + "user_pipelines/public?pipeline_name=" + this.pipeline_selected;
                const options = {
                    method: 'POST',
                    headers: {
                        'Authorization': 'Token ' + localStorage.getItem("authorization_token")
                    ...
                    url,
                    data: {public: !this.loaded_pipeline.public}
                };
                axios(options).catch(error => {
                }).then(() => {
            ...
            load_users_pipelines() {
                ...
                axios(options).catch(error => {
                    ...
                    this.loaded_pipelines.map(elem => Object.assign(elem, {display_name: elem.name +
                            " [" + elem.owner + "]" +
                            " (" + elem.type + ")"}));
                });
            },
            ...
            duplicate_pipeline() {
                ...
                const options = {
                    ...
                    data: {
                        ...
                        pipeline_owner: this.loaded_pipeline.owner
                    },
                };
                axios(options).catch(error => {
                    console.log(error);
