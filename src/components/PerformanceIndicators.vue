        ...
        <ul class="collection with-header">
            <li class="collection-header">
                <h4>
                    ...
                    <v-icon large class="right">art_track</v-icon>
                </h4>
            </li>
  ...
  export default {
    ...
    methods: {
      save: _.debounce(function () {
          if (this.can_save) {
              let url = api_url + "user_pipelines/save/performance_indicators?pipeline_name=" + this.pipeline_name;
              let saved_elements = [];
      ...
      reload_users_parameters () {
        let url = api_url + "user_pipelines/load/performance_indicators?pipeline_name=" + this.pipeline_name;
        const options = {
          method: 'GET',
          headers: {
            'Authorization': 'Token ' + localStorage.getItem("authorization_token")
    ...
    mounted() {
      let url = api_url + "get/performance_indicators?pipeline_name=" + this.pipeline_name;
      const options = {
        method: 'GET',
        headers: {
          'Authorization': 'Token ' + localStorage.getItem("authorization_token")
