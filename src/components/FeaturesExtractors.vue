    ...
    <div>
        <ul class="collection with-header">
            <li class="collection-header">
                <h4>
                    Features extractors
                    <v-icon large class="right">equalizer</v-icon>
                </h4>
            </li>
  ...
  export default {
    ...
    methods: {
      save: _.debounce(function () {
          if (this.can_save) {
              let url = api_url + "user_pipelines/save/features_extractors?pipeline_name=" + this.pipeline_name;
              let saved_elements = [];
      ...
      reload_users_parameters () {
        let url = api_url + "user_pipelines/load/features_extractors?pipeline_name=" + this.pipeline_name;
        const options = {
          method: 'GET',
          headers: {
            'Authorization': 'Token ' + localStorage.getItem("authorization_token")
    ...
    mounted() {
      let url = api_url + "get/features_extractors?pipeline_name=" + this.pipeline_name;
      const options = {
        method: 'GET',
        headers: {
          'Authorization': 'Token ' + localStorage.getItem("authorization_token")
